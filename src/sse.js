...
class SSEManager {
  ...
  broadcast(event) {
    ...
    for (const [id, res] of this.clients) {
      try { res.write(message); sent++; }
      catch (err) { this.clients.delete(id); }
    }
  ...
  getStatus() {
    return { clients: this.clients.size, uptime: Math.floor(process.uptime()) };
  }

  destroy() {
    clearInterval(this.heartbeatInterval);
    for (const [id, res] of this.clients) { try { res.end(); } catch {} }
    this.clients.clear();
  }
}
