...
async function upsertChat(chatJid, chatName, lastMessage, lastTimestamp) {
  ...
  try {
    ...
    const ts = lastTimestamp
      ...
      : new Date().toISOString();
    const { error } = await supabase.from('oasis_wa_chats').upsert({
...
async function getChats(limit = 100) {
  if (!supabase) return [];
  try {
    const { data, error } = await supabase.from('oasis_wa_chats').select('*').order('last_timestamp', { ascending: false }).limit(limit);
    if (error) throw error;
    return data || [];
...
async function getMessages(chatJid, limit = 50, before = null) {
  if (!supabase) return [];
  try {
    let query = supabase.from('oasis_wa_messages').select('*').eq('chat_jid', chatJid).order('timestamp', { ascending: false }).limit(limit);
...
async function syncInitialChats(chatsData) {
  ...
  for (const chat of chatsData) {
    try {
      await upsertChat(chat.jid, chat.name, chat.lastMessage, chat.lastTimestamp);
      if (chat.messages && chat.messages.length > 0) {
        for (const msg of chat.messages) { await saveMessage(chat.jid, chat.name, msg); }
      }
      synced++;
