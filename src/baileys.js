...
async function connectToWhatsApp() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      sseManager?.broadcast({ type: 'connection', data: { status: 'disconnected', reason } });
    ...
    if (connection === 'open') {
      ...
      sseManager?.broadcast({ type: 'connection', data: { status: 'connected' } });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    for (const msg of messages) {
      ...
      await saveMessage(chatId, senderName, {
        messageId: msg.key.id, text: messageText, type: messageType, fromMe,
        timestamp: timestamp ? (typeof timestamp === 'object' ? timestamp.low || timestamp : timestamp) : Math.floor(Date.now() / 1000)
      });
  ...
  sock.ev.on('contacts.update', (updates) => {
    for (const u of updates) { if (u.notify) contactCache.set(u.id, u.notify); }
  });
...
async function runInitialSync() {
  ...
  sseManager?.broadcast({ type: 'sync_start', data: { message: 'Sincronizando ultimos 15 chats...' } });
  ...
  try {
      ...
      for (const msg of messages) {
        ...
        if (!syncedChats.has(chatId)) {
          syncedChats.set(chatId, {
            jid: chatId, name: msg.pushName || contactCache.get(chatId) || chatId.split('@')[0],
            lastMessage: extractText(msg) || '[media]', lastTimestamp: msg.messageTimestamp, messages: []
          });
        ...
        if (chatData.messages.length < 20) {
          chatData.messages.push({
            messageId: msg.key.id, text: extractText(msg), type: getMessageType(msg),
            fromMe: msg.key.fromMe || false, timestamp: msg.messageTimestamp
          });
        }
        if (syncedChats.size >= 15) { clearTimeout(syncTimeout); finishSync(); return; }
      }
    ...
    async function finishSync() {
      ...
      if (syncedChats.size === 0) {
        ...
        sseManager?.broadcast({ type: 'sync_complete', data: { synced: 0, message: 'Sin historial previo.' } });
        return;
      }
  ...
  } catch (err) { console.error('Error en sync inicial:', err.message); initialSyncDone = true; }
}
...
function extractText(msg) {
  ...
  return inner.conversation || inner.extendedTextMessage?.text || inner.imageMessage?.caption
    ...
    || inner.templateButtonReplyMessage?.selectedDisplayText || null;
}
...
async function getProfilePhoto(jid) {
  ...
  try {
    ...
    return url || null;
  } catch { photoCache.set(jid, null); return null; }
}

function getContactName(jid) { return contactCache.get(jid) || null; }
...
async function sendMessage(chatId, content) {
  ...
  await saveMessage(chatId, 'Sanate Bot', { messageId: sent.key.id, text: sentText, type: 'text', fromMe: true, timestamp: Math.floor(Date.now() / 1000) });
  await upsertChat(chatId, null, sentText, Math.floor(Date.now() / 1000));
  sseManager?.broadcast({ type: 'message_sent', data: { chatId, messageId: sent.key.id, text: sentText, timestamp: Date.now() } });
  return sent;
}

async function disconnect() {
  if (sock) { await sock.logout(); sock = null; connectionState = 'disconnected'; initialSyncDone = false; }
}

module.exports = { initBaileys, getSocket, getQR, getConnectionState, getProfilePhoto, getContactName, sendMessage, disconnect, contactCache, photoCache };
