/* ═══════════════════════════════════════════════════════════════
   SANATE HOTFIX v6.2 — Fix Encuesta → Texto plano
   Se carga como primer script en extraScripts
   ═══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  if (window.__sanateHotfixV62) return;
  window.__sanateHotfixV62 = true;

  /* ── FETCH INTERCEPTOR: convierte text→buttons vacíos → text fallback en servidor ── */
  ...
  window.fetch = function (url, opts) {
    try {
      if (typeof url === 'string' &&
        ...
        var body;
        try { body = JSON.parse(opts.body); } catch (e) { return _origFetch(url, opts); }
        if (body && body.message && (!body.type || body.type === 'text') && !body.mediaUrl) {
          /* v6.2: type=buttons + buttons=[] → el servidor cae al fallback de texto plano */
          /* IMPORTANTE: NO borrar body.message — el servidor lo requiere para validación  */
          body.type    = 'buttons';
          body.caption = String(body.message);
          body.buttons = [];
          return _origFetch(url, Object.assign({}, opts, { body: JSON.stringify(body) }));
        }
      }
  ...
  function conectarSSE() {
    try {
      ...
      console.log('[Sánate v6.2] SSE conectado');
  ...
  function actualizarSidebar(chatId, estado) {
    ...
    document.querySelectorAll(
      ...
      if ((el.innerText || el.textContent || '').replace(/\D/g, '').includes(digits)) {
        setEtiqueta(el, estado); found = true;
      }
    });
    if (!found) {
  ...
  function setEtiqueta(item, estado) {
    ...
    if (!etiqueta) {
      ...
      var nombre = item.querySelector('[class*="name"],[class*="title"],[class*="Name"],[class*="Title"],strong,b') || item.firstElementChild;
  ...
  function aplicarClaseTick(el, statusName) {
    ...
    var map = { sent: 'sanate-status-sent', delivered: 'sanate-status-delivered', read: 'sanate-status-read', played: 'sanate-status-read', pending: 'sanate-status-pending', error: 'sanate-status-error' };
  ...
  function reescanearTicks() {
    ...
    document.querySelectorAll('span,i,em').forEach(function (s) {
      var t = s.textContent || '';
      if ((t === '\u2713' || t === '\u2713\u2713' || t === '\u2714' || t === '\u2714\u2714') && !s.classList.contains('sanate-tick-text')) s.classList.add('sanate-tick-text');
    });
  }
  ...
  console.log('[Sánate v6.2] ✅ fetch interceptor activo — encuestas → texto plano');
})();
...
console.log('[WABA Connect UI] v1.0 + Sánate fix v6.2 cargado');
})();
